...
projection and the many small lattice evaluations. Regarding standard deviation, both agents perform comparable.\\
Note that all of the results are obtained by one run with the respective hyperparameters. 
A more extensive hyperparameter search could improve the results for both networks, but is out of scope of this work.
Additionaly, it may be possible to track out-of training performance to stop training early, avoiding 
overfitting and potentially reducing the running time. Stopping with a hard cut of a certain number of episodes
may lead to a supotimal result, as the training may not have converged yet or the network is in a bad state at the time of stopping.