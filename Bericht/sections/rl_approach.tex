...
\textbf{Comparison with the state/action space formulation given by \textcite{drausp_lion18}.}
They also formulate \acs{drausp} as an \acs{mdp}, but with a considerably
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.20\textwidth}p{0.36\textwidth}p{0.36\textwidth}}
    \toprule
     & \textbf{\textcite{drausp_lion18}} & \textbf{Our Approach} \\
    \midrule
...
We use the standard DQN architecture as a baseline which tries to learn the action-value function $Q(s,a)$ without any structural knowledge about the problem.
\subsubsection{Loss function}
The parameters $\theta_{DQN}$ of the DQN are updated by minimizing a loss function. To stabilize training, the bootstrapped target is computed with a separate \emph{target network} $\theta_{DQN}^-$, which is not trained directly but slowly tracks $\theta_{DQN}$ via a soft update $\theta_{DQN}^- \leftarrow \tau\,\theta_{DQN} + (1-\tau)\,\theta_{DQN}^-$ after every optimization step. We use the standard Bellman update rule:
\[
...
\[
  L(\theta_{DQN}) = \frac{1}{|\mathcal{D'}|}\sum_{(s_t, a_t, r_{t+1}, s_{t+1}) \in \mathcal{D'}} \Big[ \big( Q(s_t, a_t \vert \theta_{DQN}) - (r_{t+1} + \gamma \max_{a'} Q(s_{t+1}, a' \vert \theta_{DQN}^-)) \big)^2 \Big],
\]
