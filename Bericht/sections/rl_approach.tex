...
\begin{itemize}
 \item $a+\ell-1 \le K$: the block does not extend beyond the last resource $K$
 \item $\hat C_{a+i-1} \ge q_i$ for all $i=1,\dots,\ell$: the capacity of all affected slots is sufficient to accept the request.
\end{itemize} 
Note that for a given state not all actions are feasible: We define the  set of feasible actions $\mathcal{A}_s^{\text{valid}} \subseteq \mathcal{A}$ as the set of actions that do not exceed the remaining capacity of any affected slot and do not exceed the horizon $K$.
...
Remark that the definition of the state and action space does not assume a fixed request length $\ell$, but allows for variable-length requests. The current request length $\ell$ is implicitly contained in the state $s$ via the demand vector $q$.
...
In case of acceptance, the new capacities are given by the following equation:
\[
  \hat C_{a+i-1} \;\leftarrow\; \hat C_{a+i-1} - q_i, \qquad i = 1,\dots,\ell.
\]
Note that $\hat{C_k} \geq 0$ for all valid actions $a \in \mathcal{A}_s^{\text{valid}}$ and all $k=1,\dots,K$.

Figure~\ref{fig:state-transition} illustrates the effect of applying the transition function $\mathcal{T}$ to the capacities for a concrete example with
$K=5$, request length $\ell=3$, and action $a=2$.
...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}
  \caption[Effect of the transition function on the resource capacities]{Effect of the transition function $\mathcal{T}$ ($K=5$ slots, request length $\ell=3$, action $a=2$): the
...
\textbf{Objective.} 
Given an MDP, the goal is to find a policy $\pi$: $S$ $\to$ $A$ that maximizes the expected cumulative reward over the decision horizon. In our case, the objective is to maximize the total revenue over the decision horizon by learning an acceptance-and-scheduling policy that maps states to actions.
\begin{equation*}
      \pi^* =\arg\max_{\pi} \mathbb{E}_{\pi}\Bigg[\sum_{t=1}^{T_d} \gamma^{t-1} \cdot \mathcal{R}(s_t, a_t)\Bigg]
  \end{equation*}
...
leaner state space. Their state space only contains the capacity vector
...
Table~\ref{tab:mdp-comparison} summarizes this structural difference.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.20\textwidth}p{0.36\textwidth}p{0.36\textwidth}}
    ...
    \bottomrule
  \end{tabular}
  \caption[State/action space: \textcite{drausp_lion18} vs.\ our approach]{Comparison of the state/action space formulation in the DRAUSP formulation of \textcite{drausp_lion18} and in our approach.}
...
\subsection{Learning the optimal policy via (deep) Q-learning}
...
\]
which should express the expected cumulative reward of taking action $a$ in state $s$ at time point $t$ and then following the optimal policy $\pi^*$ thereafter.
Once $Q^*$ is known (or
approximated by a trained network), the optimal policy acts greedily with
respect to it:
\[
...
The function approximator is a feedforward neural network with parameters $\theta_{DQN}$ that takes a state $s$ as input and outputs the Q-values for all actions $a \in \mathcal{A}$. We refer to this as the standard DQN architecture.
This serves as a baseline which tries to learn the action-value function $Q(s,a)$ without any structural knowledge about the problem.
\subsubsection{Loss function}
The parameters $\theta_{DQN}$ of the DQN are updated by minimizing a loss function. To stabilize training, the bootstrapped target is computed using a separate \emph{target network} with parameters $\theta_{DQN}^-$. Instead of being trained directly, $\theta_{DQN}^-$ slowly tracks $\theta_{DQN}$ through a soft update $\theta_{DQN}^- \leftarrow \tau\,\theta_{DQN} + (1-\tau)\,\theta_{DQN}^-$, performed after every optimization step. We use the standard Bellman update rule:
\[
  Q(s_t, a_t) \leftarrow r_{t+1} + \gamma \max_{a'} Q(s_{t+1}, a' \vert \theta_{DQN}^-),
\]
...
we ran into the problem that the agent often chooses invalid actions. Despite penalizing invalid actions with a highly negative reward, the agent
still chooses invalid actions in upcoming decision epochs. This is because the agent does not know which actions are valid and needs to learn this from the reward signal too.
...
\subsubsection{$\epsilon$-Greedy and Adaptations}
For balancing exploitation and exploration, we use an $\epsilon$-greedy policy during the training phase, which selects a random action with probability $\epsilon$ and the action given by the current Q-function with probability $1-\epsilon$.
...
with probability $\frac{1}{|\mathcal{A}_s^{\text{valid}}|} = \frac{1}{K-\ell+2}$, where $\ell$ is the length of the current request.
Note that the probability of rejecting a request (e.g. selecting action $0$) is $\frac{1}{K-\ell+2}$, which is rather small for small requests. This leads to
...
We will never have the chance to accept a request that is deep in the future if we accept too many requests at the beginning.\\
...
We define a parameter $\alpha \in [0,1]$ which defines the probability of selecting the reject action in the exploration phase. For $a \in \mathcal{A}_s^{\text{valid}}$, the resulting exploration distribution is given by
\[
  P(a) = \begin{cases}
    \alpha, & \text{if } a = 0 \\[4pt]
    \dfrac{1-\alpha}{|\mathcal{A}_s^{\text{valid}}|-1} = \dfrac{1-\alpha}{K-\ell+1}, & \text{if } a \in \mathcal{A}_s^{\text{valid}} \setminus \{0\}
  \end{cases}
\]
\begin{comment}
...
Instanzgenerator wird sie gleichverteilt aus
