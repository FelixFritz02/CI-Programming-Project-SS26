...
\begin{equation*}
      \pi^* =\arg\max_{\pi} \mathbb{E}_{\pi}\Bigg[\sum_{t=1}^{T_d} r_t\Bigg]
  \end{equation*}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.20\textwidth}p{0.36\textwidth}p{0.36\textwidth}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Comparison of the state/action space formulation in the DRAUSP formulation of \textcite{drausp_lion18} and in our approach.}
...
\[
  Q^*(s,a) = \max_\pi \; \mathbb{E}\Big[\, \sum_{t'=t}^{T_d} \cdot \mathcal{R}(s_{t'}, a_{t'}) \;\Big|\; s_t = s,\, a_t = a \,\Big],
\]
...
\[
  Q(s_t, a_t) \leftarrow r_{t+1} + \max_{a'} Q(s_{t+1}, a' \vert \theta_{DQN}^-),
\]
and define the loss function as the mean squared error between the current Q-value and the target Q-value:
\[
  L(\theta_{DQN}) = \sum_{(s_t, a_t, r_{t+1}, s_{t+1}) \in \mathcal{D'}} \Big[ \big( Q(s_t, a_t \vert \theta_{DQN}) - (r_{t+1} + \max_{a'} Q(s_{t+1}, a' \vert \theta_{DQN}^-)) \big)^2 \Big],
\]
