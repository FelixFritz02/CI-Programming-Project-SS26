...
\begin{equation*}
      \pi^* =\arg\max_{\pi} \mathbb{E}_{\pi}\Bigg[\sum_{t=1}^{T_d}\gamma^{t-1} r_t\Bigg]
  \end{equation*}
...
Note that the probability of rejecting a request is $\frac{1}{K-\ell+2}$, which is rather small for small requests. This leads to
...
\[
  P(a) = \frac{1-\alpha}{|\mathcal{A}_s^{\text{valid}}|-1} = \frac{1-\alpha}{K-\ell+1}.
\]
